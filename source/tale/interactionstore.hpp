    ...
    private:
        ...
        bool ReadJsonValueFromDictionary(T1 &out_value, nlohmann::json json, std::string key, bool required, std::string error_preamble = "")
        {
            if (!json.contains(key))
            {
                if (required)
                {
                    TATTLETALE_ERROR_PRINT(fmt::format("{}JSON DID NOT CONTAIN KEY {}", error_preamble, key));
                    return false;
                }
            }
            else
            {
                ...
                {
                    TATTLETALE_ERROR_PRINT(fmt::format("{}VALUE FOR KEY {} WAS OF AN INCORRECT TYPE", error_preamble, key));
                    return false;
                }
            }
            return true;
        ...
        bool ReadJsonValueFromArray(T1 &out_value, nlohmann::json json, size_t index, bool required, std::string error_preamble = "")
        {
            if (json.size() <= index)
            {
                if (required)
                {
                    TATTLETALE_ERROR_PRINT(fmt::format("{}JSON DID NOT CONTAIN INDEX {}", error_preamble, index));
                    return false;
                }
            }
            else
            {
                ...
                {
                    TATTLETALE_ERROR_PRINT(fmt::format("{}VALUE FOR INDEX {} WAS OF AN INCORRECT TYPE", error_preamble, index));
                    return false;
                }
            }
            return true;
